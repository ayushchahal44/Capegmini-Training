...
BASE_URL = "http://localhost:8090/gateway"
AUTH_URL = f"{BASE_URL}/auth"
APP_URL = f"{BASE_URL}/applications"
DOC_URL = f"{BASE_URL}/documents"
ADMIN_URL = f"{BASE_URL}/admin"

print("--- FinFlow Gateway API Testing ---")
...
def test_flow():
    ...
    print("\n7. Testing List My Applications...")
    resp = requests.get(f"{APP_URL}/my", headers=headers)
    print(f"List My Status: {resp.status_code}")
    if resp.status_code == 200:
        print(f"Applications Found: {len(resp.json())}")
    else:
        print(f"Error: {resp.text}")
